...
function saveNote(edit) {
    ...
    if (myContent != "" && subjectEl.value != "") {
        if (!edit) {
            ...
            if (
                myContent.localeCompare(
                    JSON.parse(localStorage.getItem(clickedDiv.id)).note
                ) === 0
            ) {
                tinymce.get("mytextarea").setContent("");
                subjectEl.value = "";
        ...
        tinymce.activeEditor.windowManager.alert(
            "Text area or/and subject is empty..! Save error..!"
        );
    }
